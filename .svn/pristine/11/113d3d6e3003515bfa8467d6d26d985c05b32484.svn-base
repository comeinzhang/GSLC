	  ...
	  <c:forEach items="${storeList }" var="store" >	
	   	  <div class="goods_info">
	           <div class="goods_info_nr">
	       	  		<img src="${ASSET_URL }${store.goods.headImgUrl}" width="66">
	                <h1 onclick="goIntoGoods('${store.goods.id}')">${store.goods.name}</h1>
	                <span class="goods_info_deleter" id="${store.id}"></span>
	                <br/>  
<!-- 	                <p style="color:#c40000;">¥ <span>${store.goods.price}</span></p> -->
	                <div class="clear"></div>
	          	</div>
	      </div>
      </c:forEach> 
