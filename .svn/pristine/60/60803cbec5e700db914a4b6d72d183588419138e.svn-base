...
<div class="container">
    <div class="tx_input mt10">
        <p class="tx_input_1">提现金额 : <input  type="text" name="pickNum" value="" onKeypress="return (/[\d.]/.test(String.fromCharCode(event.keyCode)))" placeholder="本次可提现${user.balance==null?0:user.balance}元"></p>
        <span>本次可提现${user.balance==null?0:user.balance}元</span>
        <div class="clear"></div>
    </div>
            ...
            <div class="account_num">
                <ul>
                	<c:forEach items="${bankList }" var="bank">
                        ...
                        <span><a href="toEditBank?id=${bank.id }&type=balance">编辑</a></span>
                        <div class="clear"></div>
                    </li>
                	</c:forEach>
                </ul>
                <a href="toEditBank?type=balance" target="_self" class="account_num_a">+ 添加账号</a>
            </div>
            <div class="clear"></div>
        </div>
        ...
        <script type="text/javascript">
            $(document).ready(function(){
                ...
                $('#pickUp').click(function(){
                   ...
                   var data={bankId:bankId,pickNum:pickNum};
                   $.ajax({
						cache: true,
						type: "POST",
						url:"saveRechargeBill",
						data:data,
						timeout:5000,
						async: false,
