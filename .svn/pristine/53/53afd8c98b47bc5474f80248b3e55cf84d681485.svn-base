...
<div class="container">
    ...
    <c:if test="${type ne 'refer' or PRO_NAME ne 'YPH'}">
    	<div style="text-align: right;background:#EE4F3B;padding-right: 10px;"><a href="myBonusDetail?type=${type }" target="_self">线上业绩明细</a><i class="iconfont icon-gengduo1 icon-por"></i></div>
    </c:if>
    <div class="brokerage_info border-bottom">
        ...
        <div class="brokerage_info_tabl1">
          <p>可提金额</p>
          <span style="font-size: 23px;">${pickUpBonus}</span>
        </div>
        <div class="clear"></div>
    </div>
     <div class="withdraw_cash_btn"> 
         <a href="toLogin2?url=applyPickUpMoney&type=${type}" target="_self">申请提现</a> 
     </div> 
    <!--下级分店-->
