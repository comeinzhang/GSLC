    ...
    <div class="goods_tj">
		...
		<div class="container submit_yf border_top buy_submit">
			<ul>
				<c:choose>
		        	<c:when test="${order.status eq 0}">
		        		<li><a href="tel:${shop.phone }">联系客服</a></li>
						<li onclick="deleteOrder('${order.orderNum}')">取消订单</li>
		        	</c:when>
		        	<c:when test="${order.status eq 1}">
		        		<li><a href="tel:${shop.phone }">联系客服</a></li>
						<li onclick="returnGoods('${order.orderNum}')">申请退货</li>
		        	</c:when>
		        	<c:when test="${order.status eq 2}">
		        		<li><a href="tel:${shop.phone }">联系客服</a></li>
						<li onclick="returnGoods('${order.orderNum}')">申请退货</li>
		        	...
		        	<c:when test="${order.status eq 3}">
		        		<li onclick="changeGoods('${order.orderNum}')">申请换货</li>
						<li onclick="returnGoods('${order.orderNum}')">申请退货</li>
		        	</c:when>
		        </c:choose>
				<!--<li>联系客服</li>
			...
			<c:choose>
	        	<c:when test="${order.status eq 0}">
	        		<div class="submit_btn" onclick="window.location.href='../goods/orderPay?order_num=${order.orderNum }'">立即支付</div>
	        	</c:when>
	        	<c:when test="${order.status eq 1}">
	        		<div class="submit_btn">等待发货</div>
	        	</c:when>
	        	<c:when test="${order.status eq 2}">
	        		<div class="submit_btn" onclick="completeOrder('${order.orderNum }')">确认收货</div>
	        	</c:when>
	        	<c:when test="${order.status eq 3}">
		...
		<script>
			...
			function returnGoods(orderNum){
					window.location.href="returnGoods?orderNum="+orderNum+"&type=0";
			}
			function changeGoods(orderNum){
					window.location.href="returnGoods?orderNum="+orderNum+"&type=1";
			}
