...
<div class="container">
    <div class="tx_input mt10">
        <p class="tx_input_1">提现金额 : <input  type="text" name="pickNum" value="" onKeypress="return (/[\d.]/.test(String.fromCharCode(event.keyCode)))" placeholder="本次可提现${user.oweNum==null?0:user.oweNum}元"></p>
        <span>本次可提现${user.oweNum==null?0:user.oweNum}元</span>
        <div class="clear"></div>
    </div>
            ...
            <div class="account_num">
                <ul>
                	<c:forEach items="${bankList }" var="bank">
                        ...
                        <span><a href="toEditBank?id=${bank.id }&type=owe">编辑</a></span>
                        <div class="clear"></div>
                    </li>
                	</c:forEach>
                </ul>
                <a href="toEditBank?type=owe" target="_self" class="account_num_a">+ 添加账号</a>
            </div>
            <div class="clear"></div>
        </div>
