...
<div class="page-warp">
    <div class="head-bar">
        <div class="t-bar">
            ...
            <div class="text">选择位置</div>
            <div class="ddr">
                <a href="../user/toCreateAddr" target="_self">新增地址</a>
            </div>

        </div>
    </div>
    <div class="bld-index">
        <div class="bld-i-head">
            <div class="map-alt ellipsis">
                <p class="s1"><img src="../static/images/dw_03.png"><span id="city">${city }</span></p>
            </div>
            <div class="search-box ">
                <input type="text" id="searchAddr" placeholder="输入关键字搜索">
                <div class="sc-btn" onclick="search();"><i class="iconfont icon-sousuo-sousuo"></i></div>
            </div>
        </div>
        <div class="bld-m-l">
            ...
            <div class="list-box">
                <div class="row">
                    <p class="p1" id="addr">${addr }</p>
                    <div class="xs" onclick="getCurrentPosition();"><img src="../static/images/bn'_06.png"></div>
                </div>
            </div>
            <div class="tt">附近位置</div>
            <div class="list-box" id="nearAddr">
                
            </div>
        </div>
        <div class="city-container none">
            <div id="showLetter" class="showLetter" style="display: none;"><span>M</span></div>
            <div class="city">
...
<script type="text/javascript">
	...
	function getNearAddrList(){
				...
				if (local.getStatus() == BMAP_STATUS_SUCCESS){
		            ...
		            for (var i = 0; i < results.length; i ++){
		                 for (var j = 0; j < results[i].getCurrentNumPois(); j ++){
		                    var title = results[i].getPoi(j).title;
		                    var point = results[i].getPoi(j).point;
		                    var lng1 = point.lng;
		                    var lat1 = point.lat;
		                    lis += "<div class='row' onClick='selectShop("+lng1+","+lat1+",\""+title+"\",\""+city+"\");'>"
	                        	+ "<p class='p1'>"+title+"</p>"
	                            + "</div>";
		                 }
					}
		...
		var myKeys = ["酒店","建筑","医院","学校","餐厅","地铁站"];
		local.searchNearby(myKeys,mPoint,1000);
	}
	function selectShop(lon,lat,addr,city){
		location.href="bldShopIndex?lon="+lon+"&lat="+lat+"&addr="+addr+"&city="+city;
	}
	function search(){
...
<script>
    $(function(){
        ...
        $('.map-alt').click(function () {
            $('.city-container').show();
        });

        //选择城市 start
        $('body').on('click', '.city-list p', function () {
            $('.map-alt .s1 span').html($(this).html());
            $('.city-container').hide();
            $("#city").text($(this).html());
            //alert($(this).html()+$(this).attr('data-id'))
        });
        //点击索引查询城市
        $('body').on('click', '.letter a', function () {
            var a = $(this);
            var s = a.html();
            //$('.city-container').scrollTop($('#' + s + '1').offset().top);
            var b=$('#' + s + '1').parents('.city-list').prevAll();
            var h =0;
            b.each(function () {
                h+=$(this).height();
            });
            console.log(h);
            $('.city-container').scrollTop(h+20);
